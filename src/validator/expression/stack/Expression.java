...
public class Expression {
    ...
    /*
    This method calls the to string method
     of the stack class and return the string
     of all the elements in teh stack
     */
    ...
    This method takes a string as an argument,
     validates its length, and then push every
      character of the string into the stack one
       by one
     */
    public void setExpression(String expression) throws
            ExpressionOverflowException {
        expression = expression.toUpperCase();
        if (expression.length() > 25) {
            throw new ExpressionOverflowException("Expression length" +
    ...
    /*
    This method checks every element of the stack and
    validates it one by one. if any invalid character
    would be found in the stack, this method would
    return false
     */
    //33 lines in this method. Out of which 9 contains "}"
    public boolean validateExpression() {
        MyStack temporaryStack = new MyStack(),
                validationStack = new MyStack();
        int numberOfOperators = 0, numberOfBrackets = 0,
                numberOfDigitsOrVariables = 0;
        while (stack.hasNext()) {
            char characterFromStack, characterFromValidationStack;
            characterFromStack = stack.pop();
            temporaryStack.push(characterFromStack);
            if (isNotABracket(characterFromStack) &&
                    !isAnOperator(characterFromStack)
                    && !isAVariable(characterFromStack) &&
                    !isANumber(characterFromStack)) {
            ...
            if (isAVariable(characterFromStack) ||
                    isANumber(characterFromStack)) {
                numberOfDigitsOrVariables++;
            }//END OF IF
            if (isAClosingBracket(characterFromStack)) {
                ...
                if (!isACompletePair(characterFromStack,
                        characterFromValidationStack)) {
                    reInsertStackValues(temporaryStack);
                    return false;
                }//END OF IF
            }// END OF IF /ELSE
    ...
    /*
    This method checks whether the given character is an
    ...
    /*
    This method checks whether the given character is an opening
     bracket ], }, ) or not
     */
    ...
    private boolean isACompletePair(char openingBracket,
                                    char closingBracket) {
    ...
    /*
    This method checks whether the given character is a
     valid number or not
     */
 ...
 */
    private boolean isAnOperator(char character) {
        return character == 42 || character == 43 ||
                character == 45 || character == 47 || character == 94;
    }

    /*
    This method checks whether the given character
    is a valid variable name or not
 */
    ...
    /*
    The functionality of this method is to re Insert all the
    characters back into the main stack which were popped out
